...
import { parseRequest, formToMicroformats } from '../lib/parsers.js';
import {
  createAuthError,
  createErrorResponse,
...
export const GET: APIRoute = async ({ request, url }) => {
  ...
  try {
    ...
    switch (query) {
      ...
      default:
        return addCorsHeaders(
          createErrorResponse(400, 'invalid_request', `Unknown query: ${query}`),
          config.security?.allowedOrigins
        );
    }
  } catch (error) {
...
function handleConfigQuery(config: any): Response {
  ...
  if (config.micropub?.mediaEndpoint) {
    const mediaUrl = new URL(config.micropub.mediaEndpoint, config.siteUrl).toString();
...
async function handleCreate(
  ...
  try {
    ...
    if (!entry.properties.content && !entry.properties.name && !entry.properties.photo) {
      return addCorsHeaders(
        createErrorResponse(
          400,
          'invalid_request',
...
async function handleAction(
  ...
  try {
    ...
    switch (action.action) {
      ...
      default:
        return addCorsHeaders(
          createErrorResponse(400, 'invalid_request', `Unknown action: ${data.action}`),
          config.security?.allowedOrigins
        );
    }
  } catch (error: any) {
 ...
 */
async function handleUpdate(action: any, verification: any, config: any): Promise<Response> {
 ...
 */
async function handleDelete(action: any, verification: any, config: any): Promise<Response> {
 ...
 */
async function handleUndelete(action: any, verification: any, config: any): Promise<Response> {
